...
Run_Model  <- function(inputdata,derivs,parameters,num.years=100,state=c(POM = 1, LMWC = 1, AGG = 1, MIC = 1, MAOM=1)) {
    ...
    #Run model
    output <- ode(y = state, times=run.steps, func= derivs, parms = parameters, method="rk4") #solve ode, return output
    return(output)
}
...
Solve_Model  <- function(inputdata,derivs,parameters,calc_eigens=0) {
    ...
    if(calc_eigens==1){
      ...
      eigens <- eigen(jacobian.full(y = state, func = derivs, parms = parameters, time = 1))
      write.csv(eigens, file= paste0("eigens_",derivs,".csv"))
    }

    #Solve steady state
    SS_output <- stode(y = state, time = SStime, func = derivs, parms = parameters, positive = TRUE)
    return(SS_output)
}